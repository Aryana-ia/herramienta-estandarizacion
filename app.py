...
def get_changes(original, modified):
    ...
    for tag, i1, i2, j1, j2 in matcher.get_opcodes():
        if tag == 'replace':
            ...
            new_phrase = ' '.join(mod_tokens[j1:j2])
            # Nota: inserción requiere contexto, se aplica al final o inicio aproximado
...
def main():
    st.title(
        "Herramienta de Estandarización de Descripciones (con Propagación de Cualquier Cambio)")
    ...
    row_index = st.selectbox("Selecciona la fila a editar:", range(
        total), format_func=lambda x: f"Fila {x+1}: {df.iloc[x]['descripcion_original'][:50]}...")
    ...
    st.write(
        f"**Descripción Original:** {current_row['descripcion_original']}")
    st.write(
        f"**Descripción Modificada Actual:** {current_row['descripcion_modificada']}")

    # Campo de edición
    new_description = st.text_area(
        "Edita la descripción modificada:", value=current_row['descripcion_modificada'])
    ...
    if st.button("Aplicar Cambio"):
        ...
        if changes:
            st.info(
                f"Cambios detectados: {changes}. Aplicando a descripciones que contengan las frases.")
        ...
        save_data(df)
        st.success(
            "Cambios guardados. Recarga la página para ver actualizaciones.")

        # Muestra preview de cambios
        st.write("Vista previa de filas afectadas (solo primeras 5):")
        affected = df[df.index !=
                      row_index] if changes else df.iloc[[row_index]]
        st.dataframe(
            affected[['id', 'descripcion_original', 'descripcion_modificada']].head(5))
